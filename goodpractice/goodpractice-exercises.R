...
diamonds %>%
  ggplot(aes(x = cut_interval(price, n = 5))) + #will create 5 bins
  geom_bar()

diamonds %>%
  ggplot(aes(x = cut_width(price, width = 5000))) + # will set width to price of 5000
  geom_bar()

diamonds %>%
  ggplot(aes(x = cut_number(price, n = 5))) + #give 5 mins with equal counts
  geom_bar()
...
diamonds %>%
  group_by(carat) %>%
  ggplot(aes(x = cut_width(price, width = 2000))) + 
  geom_bar()+
...
diamonds %>%
  ...
  ggplot(aes(x = cut_width(price, width = 2000))) + 
  geom_bar()+
  facet_wrap(~cut_number(carat, n = 5),scales = "free")
...
a + scale_x_log10() + scale_y_log10() + annotation_logticks() #adds the log ticks
...
co2 %>% ggplot(aes(x = Year, y = co2)) + 
  geom_line(aes(colour = Month)) +
  labs(y = expression(paste(CO[2]," level (ppm)")), title = expression(paste("Level of ", CO[2], " in Earth's atmosphere")), 
       subtitle = "Levels of carbon dioxide in the atmosphere from 1959 to 1997")
...
dep <- diamonds %>% sample_n(10)

dep %>%
  ggplot(aes(x = carat, y = price)) + 
  geom_point()+
  geom_label(data = subset(dep, price == max(price)), aes(carat, price, label = c("Highest price")))
